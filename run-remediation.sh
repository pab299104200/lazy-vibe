...
command_is_unscoped_broad_native_test() {
  local command="$1" lower
  lower="$(printf '%s' "$command" | tr '[:upper:]' '[:lower:]')"
...
evidence_command_is_allowed() {
  ...
  command_is_unscoped_broad_native_test "$command" && return 1
  ...
  if [[ "$status" == "0" ]]; then
    ...
    if ((${#test_cmds[@]} > 0)); then
      ...
      for test_cmd in "${test_cmds[@]}"; do
        ...
        if command_is_unscoped_broad_native_test "$test_cmd"; then
          {
            printf '\n$ %s\n' "$test_cmd"
            printf '[native-test] refused unscoped broad verification command; use a focused test file, node id, or mark broad/browser evidence pending.\n'
          } >> "$test_log"
...
manual_triage_reason() {
  local unit_id="$1" category="$2" findings="$3"
  if [[ -s "$findings" ]]; then
    local preferred_type=""
    case "$category" in
      blocked) preferred_type="blocked" ;;
    ...
    awk -F '\t' -v preferred="$preferred_type" '
      NR > 1 && $1 != "" {
        if ($3 == preferred) {
          printf "%s:%s: %s Required fix: %s\n", $4, $5, $6, $7
          found = 1
          exit
        }
        if (fallback == "") {
          fallback = $4 ":" $5 ": " $6 " Required fix: " $7
        }
      }
      END {
  ...
  if [[ -s "$verifier" ]]; then
    awk '
      /^[[:space:]-]*(\*\*)?Decision/ || /^[[:space:]-]*(\*\*)?Implementation decision/ || /^[[:space:]-]*(\*\*)?Launch evidence decision/ {
        gsub(/^[[:space:]-]+/, "")
        print
      }
    ' "$verifier" | paste -sd' ' -
  ...
  printf 'No verifier artifact or findings TSV exists for this unit.\n'
}

manual_triage_next_action() {
  local category="$1"
  case "$category" in
    blocked)
      printf 'External input, access, dependency, or human decision is required before another agent pass is useful.\n'
      ;;
    test_harness)
      printf 'Do not rerun broad product-code remediation blindly. First identify the exact targeted command, fixture, credential, timeout, or harness/environment fix required.\n'
      ;;
    contract_conflict)
      printf 'Resolve the product/security contract first: decide whether code, docs, tests, or packet text is authoritative, then run a targeted revision.\n'
      ;;
    split_required)
...
write_manual_triage_artifacts() {
  ...
  mkdir -p "$REMEDIATION_DIR/artifacts"
  {
    ...
    printf 'Generated by `run-remediation.sh` from `%s`.\n\n' "$queue"
    printf 'These units are intentionally not auto-revised because the verifier classified them as manual buckets. Each row includes the concrete failure reason extracted from the verifier findings TSV or verifier artifact.\n\n'
    printf '| Unit | Category | Packets | Failure reason | Next action |\n'
    printf '|---|---|---|---|---|\n'
  } > "$index"

  [[ -s "$queue" ]] || return 0

  local unit_id group model_class packets category finding_count verifier findings reason action triage_file
  while IFS=$'\t' read -r unit_id group model_class packets category finding_count verifier findings; do
    [[ "$unit_id" == "unit_id" || -z "${unit_id:-}" ]] && continue
    manual_triage_category "$category" || continue
    reason="$(manual_triage_reason "$unit_id" "$category" "$findings" | tr '\n' ' ' | sed -E 's/[[:space:]]+/ /g; s/^[[:space:]]+//; s/[[:space:]]+$//')"
    action="$(manual_triage_next_action "$category" | tr '\n' ' ' | sed -E 's/[[:space:]]+/ /g; s/^[[:space:]]+//; s/[[:space:]]+$//')"
    [[ -n "$reason" ]] || reason="No reason was available in verifier artifacts."
    triage_file="$REMEDIATION_DIR/artifacts/triage-$unit_id.md"
    {
      printf '# Manual Triage: %s\n\n' "$unit_id"
      printf -- '- Category: `%s`\n' "$category"
      printf -- '- Group: `%s`\n' "$group"
      printf -- '- Model class: `%s`\n' "$model_class"
      printf -- '- Packets: `%s`\n' "$packets"
      printf -- '- Finding count: `%s`\n' "$finding_count"
      printf -- '- Verifier artifact: `%s`\n' "$verifier"
      printf -- '- Findings TSV: `%s`\n\n' "$findings"
      printf '## Failure Reason\n\n%s\n\n' "$reason"
      printf '## Required Next Action\n\n%s\n' "$action"
    } > "$triage_file"
    printf '| `%s` | `%s` | `%s` | %s | %s |\n' \
      "$unit_id" "$category" "$packets" "$reason" "$action" >> "$index"
  done < "$queue"
}
