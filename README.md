...
- `00-run-summary.tsv` — deterministic status for manifest jobs, dynamic deep dives, and optional load test.
- `failed-jobs-next-actions.md` — non-pass jobs with log paths, likely failure markers, and focused rerun commands.
...
If a runner/provider outage is detected from job logs, the harness records `runner-unavailable.txt`, stops launching additional jobs, and marks the failed job as `RUNNER_UNAVAILABLE` in the summary. Rerun with another `RUNNER` or after provider recovery.
