...
The UX harness does not require a founder-authored script for every workflow. It combines the existing product profile with an optional, deliberately small `ux-profile.md`, then discovers routes, roles, domain objects, visible actions, and lifecycle transitions to infer a maximum of 12 high-value journeys. The launcher enforces that limit deterministically (`UX_MAX_JOURNEYS`, default `12`) so an overproducing planner cannot silently expand audit cost or dilute execution depth.
...
Execution evidence is lane-owned under `artifacts/<execution-job>/journeys/<journey-id>/`. Phase 2 allocates every journey to exactly one execution job; the launcher canonicalizes supported lane aliases, rejects unknown or duplicate allocations, injects only that job's plan rows, validates its `journey-results.tsv`, rejects missing or foreign journeys and false PASS results, and builds `artifacts/journey-evidence-index.tsv` only from validated lanes. Fixture and completion-oracle status are mandatory, so an empty-state walkthrough cannot masquerade as an executed lifecycle. Before any agents run, the harness resolves the product's installed Playwright version, installs its matching Chromium revision when needed, launches it outside the agent sandbox, probes the deployed entry point, authenticates when credentials are configured, and writes `artifacts/browser-preflight/{summary.md,summary.json,entry-page.png,auth-state.json}`. Failed entry or authentication preflight stops the run before it spends agent time. Codex simulation jobs receive an official Playwright MCP server configured with that exact Chromium executable, the verified storage state, the profile-designated dotenv file for response redaction, and a job-specific output directory. Authentication values are entered during deterministic preflight rather than exposed through agent tool arguments. The preflight supports `email_selector`, `password_selector`, and `submit_selector` overrides in `docs/ux/.creds` for nonstandard login forms. Non-interactive Codex requires `--dangerously-bypass-approvals-and-sandbox` to authorize external MCP calls; the wrapper applies it only to simulation jobs and the launcher enforces a repository snapshot around those jobs, failing any source-tree modification outside `RUN_DIR`. Discovery and evaluation remain sandboxed, browser-free, and read-only.

Failed UX execution lanes must be rerun through the generated `run-ux-audit.sh --profile ... --only ...` command, not by invoking `run-audit.sh` directly. The wrapper prepares current actor fixtures before an `--only` simulation starts, removes only that lane's stale report and evidence, stamps an execution-start marker, and rejects manifests or traces older than that marker. A simulation may report overall `PASS` only when every assigned journey is `PASS`; blocked, failed, or unverified rows keep the lane non-passing.

Products that automatically complete SSO can set `authenticated_selector` in `docs/ux/.creds` when their signed-in shell does not expose a standard `nav`, `[role="navigation"]`, or `[data-app-shell]` element. Preflight accepts an existing session only when that shell is visible on the deployed product origin and no password field is visible.
...
Node package and Playwright browser installation is lock-serialized inside the tooling directory. Readiness markers are written only after each installation succeeds, so parallel journey jobs cannot consume a partially populated package or browser directory. When a fixture provider writes a mode-restricted `artifacts/ux-fixtures/auth-state-secondary.json`, the harness gives that already-authenticated state to the isolated `playwright_secondary` MCP browser and never exposes the credential to journey agents. Otherwise, when a product supplies `docs/ux/.creds.secondary`, the harness falls back to a Node-compatible `.mjs` initializer built from the shared authentication contract. Products do not need to maintain an auth initializer wrapper.
