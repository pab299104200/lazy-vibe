...
Node package and Playwright browser installation is lock-serialized inside the tooling directory so parallel journey jobs cannot corrupt shared audit tooling. When a product supplies both `docs/ux/.creds.secondary` and `scripts/ux-browser-auth-secondary-init.ts`, simulation jobs also receive an isolated `playwright_secondary` MCP browser for workflows that require a genuinely separate signed-in identity.
